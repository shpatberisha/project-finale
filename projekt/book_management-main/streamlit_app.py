...
def sneakers_dashboard(api_key):
    ...
    if not brands:
        ...
        with st.form("add_sneaker_form"):
            ...
            with col2:
                release_year = st.number_input("Release Year", min_value=1970, max_value=datetime.now().year,
                                               value=datetime.now().year)
                categories = st.text_input("Categories (comma-separated)",
                                           placeholder="e.g., Basketball, Lifestyle, Retro")
        ...
        with col1:
            ...
            if selected_sneaker:
                with st.form("update_sneaker_form"):
                    ...
                    new_price = st.number_input("Price ($)", value=float(selected_sneaker['price'] or 0), min_value=0.0,
                                                step=10.0)
                    new_year = st.number_input("Year",
                                               value=int(selected_sneaker['release_year'] or datetime.now().year),
                                               min_value=1970, max_value=datetime.now().year)
